...
export function PoolConfigStep({ data, updateData, onNext }: StepProps) {
  ...
  // Only valid (filled-in) pools are checked — empty custom slots are not yet
  // distinct and should not block Continue before the user types anything.
        ...
        <div className="space-y-4">
          ...
          {fallbacks.map((fp, index) => (
            <div key={index} className="space-y-4 p-5 rounded-xl border border-border bg-muted/20">
              ...
              <PoolPicker
                pools={knownPoolsForSlot(pools, primary, fallbacks, index)}
                value={fp}
