...
export function AddMinerModal({ open, isJdMode, onClose, onRefresh }: AddMinerModalProps) {
  ...
  useEffect(() => {
    if (open) setActiveTab('automatic');
  }, [open]);
        ...
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Point miners manually or find compatible SV1 miners automatically.
            </p>
          </div>
        ...
        <div className="space-y-5 p-5">
          ...
          {activeTab === 'automatic' ? (
            <>
              ...
              {translatorEndpointReady ? (
                ...
                <div className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-500">
                  Could not detect a miner-reachable dashboard address. Set
            ...
            <div className="space-y-3">
              <div>
                <div className="text-sm font-medium">Point miners manually</div>
                <div className="text-xs text-muted-foreground">
                  Copy the endpoint that matches the miner's firmware.
                </div>
              </div>
          ...
          <div className="flex items-center justify-between border-t border-border pt-4">
            <div className="text-sm text-muted-foreground">
              {activeTab === 'automatic' ? `${selectedFound.length} of ${found.length} selected` : 'Manual setup'}
            </div>
