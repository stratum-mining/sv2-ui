...
test('translator config enables payout verification for solo pool mining', () => {
  const config = generateTranslatorConfig({
    ...
    miningMode: 'solo',
    mode: 'no-jd',
    pool: { ...BASE_DATA_30.pool!, user_identity: 'tb1qexample' },
  });
...
test('translator config disables payout verification for full donation solo identities', () => {
  const config = generateTranslatorConfig({
    ...
    miningMode: 'solo',
    mode: 'no-jd',
    pool: { ...BASE_DATA_30.pool!, user_identity: 'sri/donate/worker1' },
  });
...
test('translator config disables payout verification when any fallback identity is full donation', () => {
  const config = generateTranslatorConfig({
    ...BASE_DATA_30,
    miningMode: 'solo',
    mode: 'no-jd',
    pool: { ...BASE_DATA_30.pool!, user_identity: 'tb1qexample' },
    fallbackPools: [{ ...FALLBACK_1, user_identity: 'sri/donate' }],
  });
