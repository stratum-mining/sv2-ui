...
export function UnifiedDashboard() {
  ...
  const shareStats = useMemo(() => {
    ...
    // Total shares submitted to Pool (last_share_sequence_number is the count)
    const extSubmitted = serverChannels.extended_channels.reduce((sum, ch) => sum + ch.last_share_sequence_number, 0);
    const stdSubmitted = serverChannels.standard_channels.reduce((sum, ch) => sum + ch.last_share_sequence_number, 0);
