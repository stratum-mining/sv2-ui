...
export function generateTranslatorConfig(data: SetupData): string {
  ...
  // no-jd is always new format, so fallbacks always carry per-upstream user_identity.
...
${useNewFormat ? `user_identity = "${primaryIdentity}"\n` : ''}${fallbackBlocks}`;
}
...
jds_port = 3334
${useNewFormat ? `user_identity = "${identity}"\n` : ''}`;
  };
...
${useNewFormat ? '' : `# User identity/username for the upstream connection
user_identity = "${jdcPrimaryIdentity}"
